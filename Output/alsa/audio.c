...
static int alsa_try_open_pcm_device(const char *device, int *open_err);
...
{
	static const char *fallback_mixer_elements[] = {
		"Master",
		"PCM",
		"Speaker",
		"Front",
	...
	if (!pcm_element)
	{
		for (i = 0; fallback_mixer_elements[i] != NULL; i++)
		{
			if (!strcmp(alsa_cfg.mixer_device, fallback_mixer_elements[i]))
				continue;
			pcm_element = alsa_get_mixer_elem(mixer,
							  (char *)fallback_mixer_elements[i],
							  0);
			if (pcm_element)
			{
				g_warning("alsa_setup_mixer(): Failed to find mixer element '%s', using '%s' instead",
					  alsa_cfg.mixer_device, fallback_mixer_elements[i]);
				g_free(alsa_cfg.mixer_device);
				alsa_cfg.mixer_device = g_strdup(fallback_mixer_elements[i]);
				break;
			}
		}
	}
	...
	if (!pcm_element)
	{
		g_warning("alsa_setup_mixer(): Failed to find a usable mixer element on card %d, using software volume",
			  alsa_cfg.mixer_card);
		alsa_cleanup_mixer();
		return -1;
	}
...
static int alsa_setup(struct snd_format *f)
{
	static const char *fallback_pcm_devices[] = {
		"default",
		"sysdefault",
		"front",
		"plughw:0,0",
	...
	if (!alsa_try_open_pcm_device(alsa_cfg.pcm_device, &err))
	{
		for (i = 0; fallback_pcm_devices[i] != NULL; i++)
		{
			if (!strcmp(alsa_cfg.pcm_device, fallback_pcm_devices[i]))
				continue;
			if (alsa_try_open_pcm_device(fallback_pcm_devices[i], &err))
			{
				g_warning("alsa_setup(): Failed to open pcm device '%s', using '%s' instead",
					  alsa_cfg.pcm_device, fallback_pcm_devices[i]);
				g_free(alsa_cfg.pcm_device);
				alsa_cfg.pcm_device = g_strdup(fallback_pcm_devices[i]);
				break;
			}
		}
	}
...
static int alsa_try_open_pcm_device(const char *device, int *open_err)
{
	...
	debug("Trying pcm device: %s", device);
	err = snd_pcm_open(&alsa_pcm, device, SND_PCM_STREAM_PLAYBACK,
			   SND_PCM_NONBLOCK);
	if (err < 0)
	{
		if (open_err)
