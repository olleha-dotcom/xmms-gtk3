...
static void conf_ok_cb(GtkButton * button, gpointer data)
{
	...
	value = gtk_adjustment_get_value(GTK_ADJUSTMENT(data));
...
static void configure(void)
{
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(conf_dialog))), label,
			   TRUE, TRUE, 0);
	gtk_widget_show(label);

	hbox = gtk_hbox_new(FALSE, 10);
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(conf_dialog))), hbox,
			   TRUE, TRUE, 10);
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_action_area(GTK_DIALOG(conf_dialog))),
			   bbox, TRUE, TRUE, 0);
