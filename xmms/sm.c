...
static guint ice_watch_id = 0;
static GIOChannel *ice_channel = NULL;
...
static gboolean ice_handler(GIOChannel *source, GIOCondition condition, gpointer data)
{
	IceConn conn = (IceConn) data;

	if (condition & (G_IO_HUP | G_IO_ERR | G_IO_NVAL))
		return FALSE;

	IceProcessMessages(conn, NULL, NULL);
	return TRUE;
}
...
const char * sm_init(int argc, char **argv, const char *previous_session_id)
{
	...
	if (smc_conn)
	{
		...
		ice_channel = g_io_channel_unix_new(IceConnectionNumber(ice_conn));
		ice_watch_id = g_io_add_watch(ice_channel, G_IO_IN | G_IO_HUP | G_IO_ERR,
