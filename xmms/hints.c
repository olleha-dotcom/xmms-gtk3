...
static Atom xmms_x11_atom(const gchar *name)
{
	return gdk_x11_atom_to_xatom_for_display(gdk_display_get_default(),
						 gdk_atom_intern(name, FALSE));
}
...
{
	long data[1];
	Atom xa_win_hints = xmms_x11_atom("_WIN_HINTS");

	data[0] = WIN_HINTS_SKIP_TASKBAR;
	XChangeProperty(gdk_x11_get_default_xdisplay(), GDK_WINDOW_XWINDOW(gtk_widget_get_window(widget)),
...
static void gnome_wm_set_window_always(GtkWidget * window, gboolean always)
{
	...
	Atom xa_win_layer = xmms_x11_atom("_WIN_LAYER");
	...
	if (GTK_WIDGET_MAPPED(window))
	{
		...
		xev.xclient.window = GDK_WINDOW_XWINDOW(gtk_widget_get_window(window));
...
static void gnome_wm_set_window_sticky(GtkWidget * window, gboolean sticky)
{
	XEvent xev;
	long state = 0;
	Atom xa_win_state = xmms_x11_atom("_WIN_STATE");
...
static gboolean gnome_wm_found(void)
{
	...
	support_check = xmms_x11_atom("_WIN_SUPPORTING_WM_CHECK");
	
	p = XGetWindowProperty(xdisplay, root, support_check,
	    ...
	    r_format == 32 && count == 1)
	{
		Window n = *(long *) prop;
			...
			p = XGetWindowProperty(xdisplay, n, support_check, 0, 1,
...
static gboolean net_wm_found(void)
{
	...
	gdk_error_trap_push();
	support_check = xmms_x11_atom("_NET_SUPPORTING_WM_CHECK");
...
static void net_wm_set_property(GtkWidget * window, char *atom, gboolean state)
{
	...
	type = xmms_x11_atom("_NET_WM_STATE");
	property = xmms_x11_atom(atom);
...
static void net_wm_set_desktop(GtkWidget * window, gboolean all)
{
	...
	if (!all)
	{
		...
		unsigned char* prop;
		Atom r_type;
			Atom current = xmms_x11_atom("_NET_WM_DESKTOP");

		p = XGetWindowProperty(gdk_x11_get_default_xdisplay(),
				       GDK_WINDOW_XWINDOW(gtk_widget_get_window(window)), current,
				       0, 1, False, XA_CARDINAL, &r_type, &r_format,
				       &count, &bytes_remain, &prop);
		...
		if (current_desktop < 0xfffffffe)
			...
			current = xmms_x11_atom("_NET_CURRENT_DESKTOP");

		p = XGetWindowProperty(gdk_x11_get_default_xdisplay(), GDK_ROOT_WINDOW(), current,
				       0, 1, False, XA_CARDINAL, &r_type, &r_format,
				       &count, &bytes_remain, &prop);
	...
	xev.xclient.type = ClientMessage;
	xev.xclient.window = GDK_WINDOW_XWINDOW(gtk_widget_get_window(window));
	xev.xclient.message_type = xmms_x11_atom("_NET_WM_DESKTOP");
	xev.xclient.format = 32;
...
static void net_wm_move_resize(GtkWidget *window, int x, int y, gboolean move)
{
	...
	type = xmms_x11_atom("_NET_WM_MOVERESIZE");
...
{
	Atom supported = xmms_x11_atom(name), r_type;
	unsigned long bremain;
	int r_format, p;

	*atoms = NULL;
	gdk_error_trap_push();
	p = XGetWindowProperty(gdk_x11_get_default_xdisplay(), GDK_ROOT_WINDOW(), supported,
