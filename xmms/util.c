					...
					GtkItemFactory *ifactory)
{
	gtk_signal_disconnect_by_func(object,
				      ...
				      ifactory);
	gtk_object_remove_data_by_id(object, quark_popup_data);
}
				       ...
				       guint mouse_button, guint32 time)
{
	...
	pos = gtk_object_get_data_by_id(GTK_OBJECT(ifactory->widget),
						quark_user_menu_pos);
	if (!pos)
	{
	...
	/*
	 * GTK3 backends may ignore legacy global coordinates for menus.
	 * Use current pointer location for consistent popup behavior.
	 */
	(void) x;
	(void) y;
	(void) mouse_button;
	(void) time;
	gtk_menu_popup_at_pointer(GTK_MENU(ifactory->widget), NULL);
#else
...
#if GTK_CHECK_VERSION(3, 0, 0)
static gboolean filebrowser_filter_func(const GtkFileFilterInfo *info, gpointer data)
{
	struct stat st;

	if (!info || !info->filename || !*info->filename)
		return FALSE;
	if (stat(info->filename, &st) == 0 && S_ISDIR(st.st_mode))
		return TRUE;
	return input_check_file((gchar *) info->filename);
}

static void filebrowser_response(GtkDialog *dialog, gint response_id, gpointer data)
{
	...
	if (response_id == GTK_RESPONSE_ACCEPT)
	{
		...
		{
			if (input_check_file((gchar *) node->data))
				playlist_add((const gchar *) node->data);
			g_free(node->data);
		}
		g_slist_free(files);
...
{
	GtkWidget *widget = GTK_WIDGET(filesel);
	gchar *text;
	struct stat buf;
	...
	{
		gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(widget), text);
		g_free(text);
		return TRUE;
	}
	g_free(text);
...
GtkWidget *util_create_filebrowser(gboolean play_button)
{
	...
	const gchar *title = play_button ? _("Play files") : _("Load files");
	const gchar *ok_caption = play_button ? _("Play") : _("Add");
	filebrowser = gtk_file_chooser_dialog_new(title,
		NULL,
		GTK_FILE_CHOOSER_ACTION_OPEN,
		_("Close"), GTK_RESPONSE_CANCEL,
		ok_caption, GTK_RESPONSE_ACCEPT,
		NULL);
	gtk_file_chooser_set_select_multiple(GTK_FILE_CHOOSER(filebrowser), TRUE);
	if (cfg.filesel_path && *cfg.filesel_path)
		gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(filebrowser), cfg.filesel_path);
	{
		GtkFileFilter *supported_filter, *all_filter;
		supported_filter = gtk_file_filter_new();
		gtk_file_filter_set_name(supported_filter, _("Supported files"));
		gtk_file_filter_add_custom(supported_filter, GTK_FILE_FILTER_FILENAME,
					   filebrowser_filter_func, NULL, NULL);
		gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(filebrowser), supported_filter);
		all_filter = gtk_file_filter_new();
		gtk_file_filter_set_name(all_filter, _("All files"));
		gtk_file_filter_add_pattern(all_filter, "*");
		gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(filebrowser), all_filter);
		gtk_file_chooser_set_filter(GTK_FILE_CHOOSER(filebrowser), supported_filter);
	}
	g_signal_connect(filebrowser, "response", G_CALLBACK(filebrowser_response), GINT_TO_POINTER(play_button));
	return filebrowser;
}
#else
#endif
