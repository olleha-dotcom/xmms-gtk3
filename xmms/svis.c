...
void svis_draw(Widget * w)
{
	...
	if (!cfg.doublesize)
	{
		...
		gdk_draw_indexed_image(gtk_widget_get_window(mainwin), mainwin_gc, svis->vs_widget.x, svis->vs_widget.y, svis->vs_widget.width, svis->vs_widget.height, GDK_RGB_DITHER_NORMAL, (guchar *) rgb_data, 38, cmap);
	}
	else /* doublesize */
	{
		...
		gdk_draw_indexed_image(gtk_widget_get_window(mainwin), mainwin_gc, svis->vs_widget.x << 1, svis->vs_widget.y << 1, svis->vs_widget.width << 1, svis->vs_widget.height << 1, GDK_RGB_DITHER_NONE, (guchar *) rgb_data, 76, cmap);
	}
