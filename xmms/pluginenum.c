...
static void scan_plugins_from_build_tree(const gchar *base_dir);
...
void init_plugins(void)
{
	...
	/*
	 * Development fallback: when running from the source tree without
	 * installing, plugins live in ./Output, ./Input, ...
	 */
	if (!ip_data->input_list || !op_data->output_list)
	{
		gchar *cwd = g_get_current_dir();
		scan_plugins_from_build_tree(cwd);
		g_free(cwd);
	}
...
static void scan_plugins_from_build_tree(const gchar *base_dir)
{
	...
	while (plugin_dir_list[dirsel])
	{
		gchar *plugin_root = g_build_filename(base_dir, plugin_dir_list[dirsel], NULL);
		DIR *dir = opendir(plugin_root);
		struct dirent *ent;

		/* In-tree plugins can also be flat in this directory. */
		scan_plugins(plugin_root);
		{
			gchar *libs_dir = g_build_filename(plugin_root, ".libs", NULL);
			scan_plugins(libs_dir);
			g_free(libs_dir);
		}
		...
		if (dir)
		{
			while ((ent = readdir(dir)) != NULL)
			{
				...
				if (ent->d_name[0] == '.')
					continue;
				subdir = g_build_filename(plugin_root, ent->d_name, NULL);
				if (stat(subdir, &st) == 0 && S_ISDIR(st.st_mode))
				{
					scan_plugins(subdir);
					libs_subdir = g_build_filename(subdir, ".libs", NULL);
					scan_plugins(libs_subdir);
					g_free(libs_subdir);
				}
				g_free(subdir);
			}
