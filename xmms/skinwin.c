...
static void skinwin_cursor_changed_cb(GtkTreeView *view, gpointer data)
{
	gint row = xmms_gtk_clist_get_selected_row(GTK_CLIST(view));
	if (row < 0)
		return;
	change_skin_event(GTK_WIDGET(view), row, 0, NULL);
}
...
void create_skin_window(void)
{
	...
	gtk_window_set_default_size(GTK_WINDOW(skinwin), 360, 420);
	...
	g_signal_connect(G_OBJECT(skinwin_list), "cursor-changed",
			 G_CALLBACK(skinwin_cursor_changed_cb), NULL);
	gtk_widget_set_size_request(skinwin_list, 250, 200);
	scrolled_win = gtk_scrolled_window_new(NULL, NULL);
	gtk_widget_set_size_request(scrolled_win, 250, 280);
...
void show_skin_window(void)
{
	...
	if (xmms_gtk_clist_has_selection(GTK_CLIST(skinwin_list)))
	{
		gtk_clist_moveto(GTK_CLIST(skinwin_list),
				 xmms_gtk_clist_get_selected_row(GTK_CLIST(skinwin_list)),
				 0, 0.5, 0.0);
		xmms_gtk_clist_set_focus_row(GTK_CLIST(skinwin_list),
					     xmms_gtk_clist_get_selected_row(GTK_CLIST(skinwin_list)));
	}
}
