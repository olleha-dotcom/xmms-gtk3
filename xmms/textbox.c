...
static void textbox_generate_xfont_pixmap(TextBox * tb, gchar *pixmaptext)
{
	...
	if (tb->tb_pixmap_width < tb->tb_widget.width)
		tb->tb_pixmap_width = tb->tb_widget.width;
	tb->tb_pixmap = gdk_pixmap_new(gtk_widget_get_window(mainwin), tb->tb_pixmap_width, tb->tb_widget.height, 24);
	gc = tb->tb_widget.gc;
	...
	mask = gdk_pixmap_new(gtk_widget_get_window(mainwin), tb->tb_pixmap_width, tb->tb_widget.height, 1);
...
static void textbox_generate_pixmap(TextBox * tb)
{
	...
	if (tb->tb_is_scrollable)
	{
		if (tb->tb_scroll_enabled && !tb->tb_timeout_tag)
		{
			...
			tb->tb_timeout_tag =
				g_timeout_add(tag, textbox_scroll, tb);
		}
	}
	else
	{
	...
	tb->tb_pixmap = gdk_pixmap_new(gtk_widget_get_window(mainwin), tb->tb_pixmap_width, 6, 24);
	gc = tb->tb_widget.gc;
