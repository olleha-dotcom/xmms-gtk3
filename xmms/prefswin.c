...
static void prefswin_ilist_selection_changed(GtkTreeSelection *selection, gpointer data);
static void prefswin_glist_selection_changed(GtkTreeSelection *selection, gpointer data);
static void prefswin_vlist_selection_changed(GtkTreeSelection *selection, gpointer data);
static void prefswin_elist_selection_changed(GtkTreeSelection *selection, gpointer data);
...
void prefswin_ilist_clicked(GtkCList *clist, gint row, gint column, GdkEventButton *event, gpointer data)
{
	...
	if (xmms_gtk_clist_has_selection(clist))
	{
		...
		index = xmms_gtk_clist_get_selected_row(clist);
...
void prefswin_iconfigure(GtkButton * w, gpointer data)
{
	...
	if (xmms_gtk_clist_has_selection(clist))
		input_configure(xmms_gtk_clist_get_selected_row(clist));
}
...
void prefswin_iabout(GtkButton * w, gpointer data)
{
	GtkCList *clist = GTK_CLIST(data);
	if (xmms_gtk_clist_has_selection(clist))
		input_about(xmms_gtk_clist_get_selected_row(clist));
}
...
static void prefswin_output_changed_cb(GtkComboBox *combo, gpointer data)
{
	...
	if (idx >= 0)
		prefswin_output_cb(GTK_WIDGET(combo), GINT_TO_POINTER(idx));
}

void prefswin_gconfigure(GtkButton * w, gpointer data)
{
	GtkCList *clist = GTK_CLIST(data);
	gint sel = xmms_gtk_clist_get_selected_row(clist);
...
static void prefswin_ilist_selection_changed(GtkTreeSelection *selection, gpointer data)
{
	...
	if (!view && GTK_IS_TREE_VIEW(data))
		view = GTK_TREE_VIEW(data);
	if (!view)
		return;
	prefswin_ilist_clicked(GTK_CLIST(view), -1, 0, NULL, NULL);
}
...
static void prefswin_glist_selection_changed(GtkTreeSelection *selection, gpointer data)
{
	...
	if (!view)
		return;
	prefswin_glist_clicked(GTK_CLIST(view), -1, 0, NULL, NULL);
}
...
static void prefswin_vlist_selection_changed(GtkTreeSelection *selection, gpointer data)
{
	...
	if (!view)
		return;
	prefswin_vlist_clicked(GTK_CLIST(view), -1, 0, NULL, NULL);
}
...
static void prefswin_elist_selection_changed(GtkTreeSelection *selection, gpointer data)
{
	...
	if (!view)
		return;
	prefswin_elist_clicked(GTK_CLIST(view), -1, 0, NULL, NULL);
}
...
void prefswin_font_browse_cb(GtkWidget * w, gpointer data)
{
	...
	gtk_signal_connect(GTK_OBJECT(gtk_font_selection_dialog_get_ok_button(GTK_FONT_SELECTION_DIALOG(fontsel))), "clicked", GTK_SIGNAL_FUNC(prefswin_font_browse_ok), fontsel);
	gtk_signal_connect_object(GTK_OBJECT(gtk_font_selection_dialog_get_cancel_button(GTK_FONT_SELECTION_DIALOG(fontsel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(fontsel));
...
void prefswin_mainwin_font_browse_cb(GtkWidget * w, gpointer data)
{
	...
	if (!fontsel)
	{
		...
		gtk_signal_connect(GTK_OBJECT(gtk_font_selection_dialog_get_ok_button(GTK_FONT_SELECTION_DIALOG(fontsel))), "clicked", GTK_SIGNAL_FUNC(prefswin_mainwin_font_browse_ok), fontsel);
		gtk_signal_connect_object(GTK_OBJECT(gtk_font_selection_dialog_get_cancel_button(GTK_FONT_SELECTION_DIALOG(fontsel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(fontsel));
		gtk_signal_connect(GTK_OBJECT(fontsel), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &fontsel);
		gtk_widget_show(fontsel);
	}
...
void prefswin_gplugins_use_cb(GtkToggleButton * w, gpointer data)
{
	...
	if (!xmms_gtk_clist_has_selection(clist) || updating_glist)
		return;

	sel = xmms_gtk_clist_get_selected_row(clist);
...
void prefswin_vplugins_use_cb(GtkToggleButton * w, gpointer data)
{
	...
	if (!xmms_gtk_clist_has_selection(clist) || updating_vlist)
		return;
...
void prefswin_eplugins_use_cb(GtkToggleButton * w, gpointer data)
{
	...
	if (!xmms_gtk_clist_has_selection(clist) || updating_elist)
		return;
	sel = xmms_gtk_clist_get_selected_row(clist);
...
void prefswin_ip_toggled(GtkToggleButton * w, gpointer data)
{
	...
	if (!xmms_gtk_clist_has_selection(clist) || updating_ilist)
		return;
...
void create_prefs_window(void)
{
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_audio_iframe), 5);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_audio_ivbox), 5);
	...
	g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(prefswin_audio_ilist)),
			 "changed", G_CALLBACK(prefswin_ilist_selection_changed), prefswin_audio_ilist);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_audio_oframe), 5);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_audio_ovbox), 5);
	gtk_container_add(GTK_CONTAINER(prefswin_audio_oframe), prefswin_audio_ovbox);

	prefswin_audio_olist = gtk_option_menu_new();
	g_signal_connect(G_OBJECT(prefswin_audio_olist), "changed",
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_eplugins_frame), 5);

	prefswin_eplugins_vbox = gtk_vbox_new(FALSE, 10);
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_eplugins_vbox), 5);
	...
	g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(prefswin_eplugins_list)),
			 "changed", G_CALLBACK(prefswin_elist_selection_changed), prefswin_eplugins_list);
	scrolled_win = gtk_scrolled_window_new(NULL, NULL);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_gplugins_frame), 5);

	prefswin_gplugins_vbox = gtk_vbox_new(FALSE, 10);
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_gplugins_vbox), 5);
	...
	g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(prefswin_gplugins_list)),
			 "changed", G_CALLBACK(prefswin_glist_selection_changed), prefswin_gplugins_list);
	scrolled_win = gtk_scrolled_window_new(NULL, NULL);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_vplugins_frame), 5);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_vplugins_vbox), 5);
	...
	g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(prefswin_vplugins_list)),
			 "changed", G_CALLBACK(prefswin_vlist_selection_changed), prefswin_vplugins_list);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_fonts_options_frame), 5);
	...
	gtk_container_set_border_width(GTK_CONTAINER(options_font_vbox), 5);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_mainwin_frame), 5);
	prefswin_mainwin_vbox = gtk_vbox_new(FALSE, 10);
	gtk_container_add(GTK_CONTAINER(prefswin_mainwin_frame), prefswin_mainwin_vbox);
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_mainwin_vbox), 5);
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_title_frame), 5);
	prefswin_title_vbox2 = gtk_vbox_new(FALSE, 10);
	gtk_container_add(GTK_CONTAINER(prefswin_title_frame), prefswin_title_vbox2);
	gtk_container_set_border_width(GTK_CONTAINER(prefswin_title_vbox2), 5);
...
void show_prefs_window(void)
{
	...
	{
		xmms_gdk_window_raise_if_valid(gtk_widget_get_window(prefswin));
		return;
	}
