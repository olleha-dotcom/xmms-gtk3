...
{
	mainwin_press(widget, event, data);
	return TRUE;
}

static gboolean mainwin_button_release_event_cb(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	mainwin_release(widget, event, data);
	return TRUE;
}

static gboolean mainwin_motion_notify_event_cb(GtkWidget *widget, GdkEventMotion *event, gpointer data)
{
...
{
	mainwin_focus_in(widget, event, data);
	return FALSE;
}

static gboolean mainwin_focus_out_event_cb(GtkWidget *widget, GdkEvent *event, gpointer data)
{
	mainwin_focus_out(widget, (GdkEventButton *) event, data);
	return FALSE;
}
...
static void read_config(void)
{
	...
	/* GTK3 migration: keep Winamp-style undecorated windows by default. */
	cfg.show_wm_decorations = FALSE;
	if (cfg.outputplugin == NULL)
	{
#ifdef __linux__
		cfg.outputplugin = g_strdup_printf("%s/%s/libpulseout.so", PLUGIN_DIR, plugin_dir_list[0]);
...
static void mainwin_ensure_geometry(void)
{
	...
	if (width != expected_w || height != expected_h)
		dock_resize(dock_window_list, mainwin, expected_w, expected_h);
}
...
void mainwin_press(GtkWidget * widget, GdkEventButton * event, gpointer callback_data)
{
	    ...
	    (cfg.easy_move || event->y < 14))
	{
		...
		{
				xmms_gdk_window_raise_if_valid(gtk_widget_get_window(mainwin));
			equalizerwin_raise();
			playlistwin_raise();
...
gboolean mainwin_keypress(GtkWidget * w, GdkEventKey * event, gpointer data)
{
	if ((event->state & GDK_CONTROL_MASK) &&
	    gdk_keyval_to_lower(event->keyval) == (guint) 'd')
	{
		GtkWidget *widget;
		gboolean new_state;

		new_state = !cfg.doublesize;
		mainwin_menurow->mr_doublesize_selected = new_state;
		draw_widget(mainwin_menurow);
		set_doublesize(new_state);

		widget = gtk_item_factory_get_widget(mainwin_options_menu, "/DoubleSize");
		if (widget && GTK_IS_CHECK_MENU_ITEM(widget))
			gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(widget), new_state);
...
{
	if (xmms_gtk_clist_has_selection(clist))
	{
		int *pos;

		if (get_input_playing())
			input_stop();
		pos = gtk_clist_get_row_data(clist, xmms_gtk_clist_get_selected_row(clist));
...
static void mainwin_jtf_set_qbtn_label(GtkCList *clist, GtkWidget *button)
{
	...
	if (xmms_gtk_clist_has_selection(clist))
	{
		int *pos, lpos = xmms_gtk_clist_get_selected_row(clist);
...
static void mainwin_jump_to_file_edit_real(GtkWidget * widget, gpointer userdata)
{
	...
	gint prev_focus = -1;
	gint *tmp = gtk_clist_get_row_data(clist, xmms_gtk_clist_get_focus_row(clist));
	if (tmp != NULL)
		prev_focus = *tmp;
	...
	if (xmms_gtk_clist_has_selection(clist))
		xmms_gtk_clist_set_focus_row(clist, xmms_gtk_clist_get_selected_row(clist));
}
...
{ 
	...
	if (xmms_gtk_clist_get_focus_row(clist)) {
		gtk_clist_thaw(clist);
		gtk_clist_moveto(clist, xmms_gtk_clist_get_focus_row(clist), -1, 0.5, 0.0);
	}
	else
	{
...
{ 
	...
	GtkCList* clist = GTK_CLIST(userdata);
	gtk_clist_freeze(clist);
	adjustment = gtk_adjustment_get_value(gtk_clist_get_vadjustment(clist));
	mainwin_jump_to_file_edit_real(widget, userdata);
	gtk_adjustment_set_value(gtk_clist_get_vadjustment(clist), adjustment);
	gtk_clist_thaw(clist);
} 
...
static void mainwin_jump_to_file_queue_cb(GtkButton * widget, gpointer userdata)
{
	...
	if (xmms_gtk_clist_has_selection(clist))
	{
		int lpos = xmms_gtk_clist_get_selected_row(clist);
		pos = gtk_clist_get_row_data(clist, lpos);
...
static void mainwin_jump_to_file(void)
{
	...
	if (xmms_gtk_clist_has_selection(GTK_CLIST(clist))) {
		gtk_clist_moveto(GTK_CLIST(clist),
			 xmms_gtk_clist_get_selected_row(GTK_CLIST(clist)),
			 0, 0.5, 0.0);
		xmms_gtk_clist_set_focus_row(GTK_CLIST(clist), xmms_gtk_clist_get_selected_row(GTK_CLIST(clist)));
	}
}
...
static void mainwin_queue_manager_select_row_cb(GtkCList * widget, gint row, gint column, GdkEventButton * event, gpointer cb_data)
{
	...
	if (event && event->button == 1 && event->type == GDK_2BUTTON_PRESS && xmms_gtk_clist_has_selection(clist)) {
		int *pos = gtk_clist_get_row_data(clist, xmms_gtk_clist_get_selected_row(clist));
...
static void mainwin_queue_manager_queue_refresh(GtkWidget * widget, gpointer userdata)
{
	...
	adjustment = gtk_adjustment_get_value(gtk_clist_get_vadjustment(qlist));

	tmp = gtk_clist_get_row_data(qlist, xmms_gtk_clist_get_focus_row(qlist));
	if (tmp != NULL) 
	...
	if (row_to_select != -1) {
		...
		xmms_gtk_clist_set_focus_row(qlist, xmms_gtk_clist_get_selected_row(qlist));
	}
	gtk_clist_thaw(qlist);
	/* make sure we don't scroll out of the list */
	if (adjustment + gtk_adjustment_get_page_size(gtk_clist_get_vadjustment(qlist)) >
	    gtk_adjustment_get_upper(gtk_clist_get_vadjustment(qlist)))
		adjustment = gtk_adjustment_get_upper(gtk_clist_get_vadjustment(qlist)) -
			     gtk_adjustment_get_page_size(gtk_clist_get_vadjustment(qlist));
	gtk_adjustment_set_value(gtk_clist_get_vadjustment(qlist), adjustment);
} 
...
static void mainwin_queue_manager_move_up_cb(GtkWidget *widget, gpointer userdata)
{
	...
	if (xmms_gtk_clist_has_selection(qlist)) {
		int *pos = gtk_clist_get_row_data(qlist,xmms_gtk_clist_get_focus_row(qlist));
		if (pos != NULL && *pos > 0)
...
static void mainwin_queue_manager_remove_cb(GtkWidget *widget, gpointer userdata)
{
	...
	if (xmms_gtk_clist_has_selection(qlist)) {
		int *tmp = gtk_clist_get_row_data(qlist, xmms_gtk_clist_get_selected_row(qlist));
...
void mainwin_eject_pushed(void)
{
	...
	if (filebrowser != NULL)
	{
		...
		xmms_gdk_window_raise_if_valid(gtk_widget_get_window(filebrowser));
		return;
	}
	...
	gtk_signal_connect(GTK_OBJECT(filebrowser), "key_press_event",
			   util_dialog_keypress_cb,  NULL);
	gtk_window_set_transient_for(GTK_WINDOW(filebrowser), GTK_WINDOW(mainwin));
	gtk_widget_show(filebrowser);
}
...
/*  		return; */
	...
	{
		nullmask = gdk_pixmap_new(gtk_widget_get_window(mainwin), 20, 20, 1);
...
void mainwin_songname_menu_callback(gpointer cb_data, guint action, GtkWidget * w)
{
	switch (action)
	{
		...
		case MAINWIN_SONGNAME_SCROLL:
			cfg.autoscroll = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_songname_menu, "/Autoscroll Song Name")));
...
void mainwin_options_menu_callback(gpointer cb_data, guint action, GtkWidget * w)
{
	...
	case MAINWIN_OPT_REPEAT:
		cfg.repeat = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/Repeat")));
	...
	case MAINWIN_OPT_TDISPLAY:
		cfg.timer_minutes_only = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/Time Display (MMM:SS)")));
		break;
	case MAINWIN_OPT_ALWAYS:
		mainwin_menurow->mr_always_selected = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/Always On Top")));
	...
	case MAINWIN_OPT_WS:
		mainwin_set_shade_menu_cb(gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/WindowShade Mode"))));
		break;
	case MAINWIN_OPT_PWS:
		playlistwin_set_shade_menu_cb(gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/Playlist WindowShade Mode"))));
		break;
	case MAINWIN_OPT_EQWS:
		equalizerwin_set_shade_menu_cb(gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/Equalizer WindowShade Mode"))));
		break;
	case MAINWIN_OPT_DOUBLESIZE:
		mainwin_menurow->mr_doublesize_selected = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/DoubleSize")));
		draw_widget(mainwin_menurow);
		set_doublesize(mainwin_menurow->mr_doublesize_selected);
		gdk_flush();
		break;
	case MAINWIN_OPT_EASY_MOVE:
		cfg.easy_move = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/Easy Move")));
		break;
	case MAINWIN_OPT_NPA:
		cfg.no_playlist_advance = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_options_menu, "/No Playlist Advance")));
	}
}
...
void mainwin_vis_menu_callback(gpointer cb_data, guint action, GtkWidget * w)
{
	...
	case MAINWIN_VIS_ANALYZER_PEAKS:
		cfg.analyzer_peaks = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_vis_menu, mainwin_vis_menu_entries[MAINWIN_VIS_MENU_ANALYZER_PEAKS].path)));
		break;
...
void mainwin_general_menu_callback(gpointer cb_data, guint action, GtkWidget * w)
{
	...
	case MAINWIN_GENERAL_SHOWMWIN:
		if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_general_menu, "/Main Window"))))
			mainwin_real_show();
		else
			mainwin_real_hide();
		break;
	case MAINWIN_GENERAL_SHOWPLWIN:
		if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_general_menu, "/Playlist Editor"))))
	...
	case MAINWIN_GENERAL_SHOWEQWIN:
		if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_general_menu, "/Graphical EQ"))))
...
static void mainwin_set_icon (GtkWidget *win)
{
	...
	if (!icon)
	{
		const gchar *candidates[] = {
			"xmms/xmms.png",
			"./xmms.png",
			DATA_DIR "/pixmaps/xmms.png",
			"/usr/share/pixmaps/xmms.png",
			NULL
		};
		int i;
		for (i = 0; !icon && candidates[i]; i++)
		{
			if (g_file_test(candidates[i], G_FILE_TEST_EXISTS))
				icon = gdk_pixbuf_new_from_file(candidates[i], NULL);
		}
		if (!icon)
			icon = gdk_pixbuf_new_from_xpm_data((const char **) xmms_mini_xpm);
	}
	if (icon)
	{
...
static void xmms_set_default_app_icon(void)
{
	...
	gtk_window_set_default_icon_name("xmms");
}

static void mainwin_create_widgets(void)
{
	mainwin_gc = gdk_gc_new(gtk_widget_get_window(mainwin));
	...
	mainwin_vis = create_vis(&mainwin_wlist, mainwin_bg, gtk_widget_get_window(mainwin), mainwin_gc, 24, 43, 76, cfg.doublesize);
...
static void mainwin_create_gtk(void)
{
	...
	gtk_window_set_decorated(GTK_WINDOW(mainwin), cfg.show_wm_decorations ? TRUE : FALSE);
	...
	gtk_widget_add_events(mainwin, GDK_FOCUS_CHANGE_MASK | GDK_BUTTON_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_STRUCTURE_MASK);
	...
	g_signal_connect(G_OBJECT(mainwin), "button-press-event",
			 G_CALLBACK(mainwin_button_press_event_cb), NULL);
	g_signal_connect(G_OBJECT(mainwin), "button-release-event",
			 G_CALLBACK(mainwin_button_release_event_cb), NULL);
	g_signal_connect(G_OBJECT(mainwin), "motion-notify-event",
			 G_CALLBACK(mainwin_motion_notify_event_cb), NULL);
	g_signal_connect(G_OBJECT(mainwin), "focus-in-event",
			 G_CALLBACK(mainwin_focus_in_event_cb), NULL);
	g_signal_connect(G_OBJECT(mainwin), "focus-out-event",
			 G_CALLBACK(mainwin_focus_out_event_cb), NULL);
	...
	/* GTK3 path: client_event handling is not wired through legacy signal API. */
...
static void output_failed(void)
{
	...
	if (!infobox)
	{
		...
		gtk_signal_connect(GTK_OBJECT(infobox), "destroy",
			...
			xmms_gdk_window_raise_if_valid(gtk_widget_get_window(infobox));
...
{	
	...
	mask = gdk_pixmap_new(gtk_widget_get_window(window), 1, 1, 1);
	gc = gdk_gc_new(mask);
	pattern.pixel = 0;
	gdk_gc_set_foreground(gc, &pattern);
...
static void enable_x11r5_session_management(int argc, char **argv)
{
	...
	if (GDK_IS_X11_DISPLAY(gdk_display_get_default()) &&
	    gtk_widget_get_window(mainwin) &&
	    GDK_IS_X11_WINDOW(gtk_widget_get_window(mainwin)))
	{
		XSetCommand(gdk_x11_get_default_xdisplay(),
			    GDK_WINDOW_XWINDOW(gtk_widget_get_window(mainwin)),
			    restart_argv, restart_argc);
	...
	if (XGetWMProtocols(gdk_x11_get_default_xdisplay(), GDK_WINDOW_XWINDOW(gtk_widget_get_window(mainwin)), &temp, &n))
	{
		for (i = 0; i < n; i++)
		...
		XSetWMProtocols(gdk_x11_get_default_xdisplay(), GDK_WINDOW_XWINDOW(gtk_widget_get_window(mainwin)), temp2, n + 1);
		if (n > 0)
			XFree(temp);
		g_free(temp2);
	}
	else
		XSetWMProtocols(gdk_x11_get_default_xdisplay(), GDK_WINDOW_XWINDOW(gtk_widget_get_window(mainwin)), &save_yourself, 1);
#endif
}
...
int main(int argc, char **argv)
{
	...
	/*
	 * XMMS skin windows rely on undecorated X11 behavior.
	 * On Wayland this is often ignored, so prefer X11/Xwayland
	 * when the backend is not explicitly configured by user.
	 */
	...
	{
		if (forced_x11_backend)
		{
			g_unsetenv("GDK_BACKEND");
			forced_x11_backend = FALSE;
			if (gtk_init_check(&argc, &argv))
				goto gtk_inited;
		}
		if (argc < 2)
