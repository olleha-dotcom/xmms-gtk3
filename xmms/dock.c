...
static void calc_snap_offset(GList *dlist, GList *wlist, gint x, gint y,gint *off_x, gint *off_y)
{
	...
	*off_y = 0;
	...
	{
		dw = dnode->data;
		gdk_window_get_size(gtk_widget_get_window(dw->w), &nw, &nh);
		...
		for (wnode = wlist; wnode; wnode = g_list_next(wnode))
		{
			...
			gdk_window_get_size(gtk_widget_get_window(w), &sw, &sh);
...
static GList *get_docked_list(GList *dlist, GList *wlist, GtkWidget *w, gint init_off_x, gint init_off_y)
{
	...
	gdk_window_get_size(gtk_widget_get_window(w),&w_width,&w_height);
	if (!dlist)
	{
	...
	for (node = wlist; node; node = g_list_next(node))
	{
		...
		gdk_window_get_size(gtk_widget_get_window(GTK_WIDGET(node->data)), &t_width, &t_height);
...
void dock_move_press(GList *window_list, GtkWidget *w, GdkEventButton *event, gboolean move_list)
{
	...
	gdkwin = gtk_widget_get_window(w);
	xmms_gdk_window_raise_if_valid(gdkwin);
	...
	gtk_object_set_data(GTK_OBJECT(w), "move_offset_x", GINT_TO_POINTER(offset_x));
	gtk_object_set_data(GTK_OBJECT(w), "move_offset_y", GINT_TO_POINTER(offset_y));
	if (move_list)
...
void dock_move_motion(GtkWidget *w, GdkEventMotion *event)
{
	...
	gdk_flush();
	/* Drop queued motion events: GTK3 backend no longer exposes GDK_DISPLAY() macro form. */
