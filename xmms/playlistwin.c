...
static void playlistwin_create_mask(void)
{
	...
	playlistwin_mask = gdk_pixmap_new(gtk_widget_get_window(playlistwin), cfg.playlist_width, PLAYLIST_HEIGHT, 1);
...
void playlistwin_raise(void)
{
	if (cfg.playlist_visible)
		xmms_gdk_window_raise_if_valid(gtk_widget_get_window(playlistwin));
}
...
static void playlistwin_resize(int width, int height)
{
	...
	playlistwin_bg = gdk_pixmap_new(gtk_widget_get_window(playlistwin), cfg.playlist_width, cfg.playlist_height,
					24);
...
static void playlistwin_show_filebrowser(void)
{
	...
	gtk_window_set_transient_for(GTK_WINDOW(filebrowser), GTK_WINDOW(playlistwin));
	gtk_widget_show(filebrowser);
}
...
static void playlistwin_show_load_filesel(void)
{
	...
	if (cfg.playlist_path)
		gtk_file_selection_set_filename(GTK_FILE_SELECTION(load_filesel),
						cfg.playlist_path);
	object = GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(load_filesel)));
	gtk_signal_connect(object, "clicked",
			   GTK_SIGNAL_FUNC(playlistwin_load_filesel_ok),
			   load_filesel);
	object = GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(load_filesel)));
...
static void playlistwin_show_save_filesel(void)
{
	...
	if (cfg.playlist_path)
		gtk_file_selection_set_filename(GTK_FILE_SELECTION(filesel),
						cfg.playlist_path);
	object = GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(filesel)));
	gtk_signal_connect(object, "clicked",
			   GTK_SIGNAL_FUNC(playlistwin_save_filesel_ok),
			   filesel);
	object = GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(filesel)));
	gtk_signal_connect_object(object, "clicked",
	...
	gtk_box_pack_start(GTK_BOX(xmms_gtk_file_selection_get_action_area(GTK_FILE_SELECTION(filesel))),
			   frame, TRUE, TRUE, 0);
...
static void playlistwin_create_gtk(void)
{
	...
	gtk_window_set_decorated(GTK_WINDOW(playlistwin), cfg.show_wm_decorations ? TRUE : FALSE);
	...
	g_signal_connect(G_OBJECT(playlistwin), "delete-event", G_CALLBACK(playlistwin_delete), NULL);
	...
	g_signal_connect(G_OBJECT(playlistwin), "selection-received", G_CALLBACK(selection_received), NULL);
