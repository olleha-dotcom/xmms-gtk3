...
void playlist_popup_destroy(void)
{
	if (popup)
	{
		if (gtk_grab_get_current() == popup->window)
...
static gboolean playlist_popup_draw_event(GtkWidget *widget, cairo_t *cr, gpointer callback_data)
{
	(void) widget;
	(void) cr;
	(void) callback_data;
		    ...
		    gint barx, gint bary, gint base, void (*handler) (gint item))
{
	...
	gtk_widget_add_events(popup->window, GDK_BUTTON_MOTION_MASK | GDK_BUTTON_RELEASE_MASK);
	gtk_window_set_transient_for(GTK_WINDOW(popup->window), GTK_WINDOW(mainwin));
	...
	g_signal_connect(G_OBJECT(popup->window), "draw", G_CALLBACK(playlist_popup_draw_event), NULL);
	g_signal_connect(G_OBJECT(popup->window), "motion-notify-event", G_CALLBACK(playlist_popup_motion), NULL);
	g_signal_connect(G_OBJECT(popup->window), "button-release-event", G_CALLBACK(playlist_popup_release), NULL);
	...
	xmms_gdk_window_raise_if_valid(gtk_widget_get_window(popup->window));
	gdk_flush();
	playlist_popup_draw(popup);
}
