...
void equalizerwin_raise(void)
{
	if (cfg.equalizer_visible)
		xmms_gdk_window_raise_if_valid(gtk_widget_get_window(equalizerwin));
}
...
void equalizerwin_motion(GtkWidget * widget, GdkEventMotion * event, gpointer callback_data)
{
	...
	if (GDK_IS_X11_DISPLAY(gdk_display_get_default()))
	{
		...
		if (xdisplay)
			while (XCheckMaskEvent(xdisplay, ButtonMotionMask, &ev)) ;
	}
...
static void equalizerwin_create_gtk(void)
{
	...
	gtk_window_set_decorated(GTK_WINDOW(equalizerwin), cfg.show_wm_decorations ? TRUE : FALSE);
	...
	g_signal_connect(G_OBJECT(equalizerwin), "button-press-event",
			 G_CALLBACK(equalizerwin_press), NULL);
	g_signal_connect(G_OBJECT(equalizerwin), "button-release-event",
			 G_CALLBACK(equalizerwin_release), NULL);
	g_signal_connect(G_OBJECT(equalizerwin), "motion-notify-event",
			 G_CALLBACK(equalizerwin_motion), NULL);
	g_signal_connect(G_OBJECT(equalizerwin), "focus-in-event",
			 G_CALLBACK(equalizerwin_focus_in), NULL);
	g_signal_connect(G_OBJECT(equalizerwin), "focus-out-event",
			 G_CALLBACK(equalizerwin_focus_out), NULL);
	g_signal_connect(G_OBJECT(equalizerwin), "configure-event",
...
static void equalizerwin_load_ok(GtkWidget * widget, gpointer data)
{
	...
	if (clist && xmms_gtk_clist_has_selection(clist))
	{
		gtk_clist_get_text(clist, xmms_gtk_clist_get_selected_row(clist), 0, &text);
...
static void equalizerwin_delete_delete(GtkWidget * widget, gpointer data)
{
	...
	row = xmms_gtk_clist_get_selected_row(clist);
	if (row < 0)
		return;
	...
	switch (action)
	{
		case EQUALIZER_PRESETS_LOAD_PRESET:
			if (!equalizerwin_load_window)
				...
				xmms_gdk_window_raise_if_valid(gtk_widget_get_window(equalizerwin_load_window));
			break;
		case EQUALIZER_PRESETS_LOAD_AUTOPRESET:
			if (!equalizerwin_load_auto_window)
				...
				xmms_gdk_window_raise_if_valid(gtk_widget_get_window(equalizerwin_load_auto_window));
			break;
		...
		case EQUALIZER_PRESETS_LOAD_FROM_FILE:
		{
			...
			gtk_signal_connect(GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(load_filesel))), "clicked", GTK_SIGNAL_FUNC(equalizerwin_load_filesel_ok), load_filesel);
			gtk_signal_connect_object(GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(load_filesel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(load_filesel));
		...
		case EQUALIZER_PRESETS_LOAD_FROM_WINAMPFILE:
		{
			...
			gtk_signal_connect(GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(load_winamp_filesel))), "clicked", GTK_SIGNAL_FUNC(equalizerwin_load_winamp_filesel_ok), load_winamp_filesel);
			gtk_signal_connect_object(GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(load_winamp_filesel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(load_winamp_filesel));
		...
		case EQUALIZER_PRESETS_IMPORT_WINAMPFILE:
		{
			...
			gtk_signal_connect(GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(import_winamp_filesel))), "clicked", GTK_SIGNAL_FUNC(equalizerwin_import_winamp_filesel_ok), import_winamp_filesel);
			gtk_signal_connect_object(GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(import_winamp_filesel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(import_winamp_filesel));
		...
		case EQUALIZER_PRESETS_SAVE_PRESET:
			if (!equalizerwin_save_window)
				equalizerwin_create_list_window(equalizer_presets, _("Save preset"), &equalizerwin_save_window, GTK_SELECTION_SINGLE, &equalizerwin_save_entry, _("OK"), _("Cancel"), equalizerwin_save_ok, equalizerwin_save_select);
					...
					xmms_gdk_window_raise_if_valid(gtk_widget_get_window(equalizerwin_save_window));
			break;
		case EQUALIZER_PRESETS_SAVE_AUTOPRESET:
		{
			...
			if (!equalizerwin_save_auto_window)
				equalizerwin_create_list_window(equalizer_auto_presets, _("Save auto-preset"), &equalizerwin_save_auto_window, GTK_SELECTION_SINGLE, &equalizerwin_save_auto_entry, _("OK"), _("Cancel"), equalizerwin_save_auto_ok, equalizerwin_save_auto_select);
					...
					xmms_gdk_window_raise_if_valid(gtk_widget_get_window(equalizerwin_save_auto_window));
		...
		case EQUALIZER_PRESETS_SAVE_TO_FILE:
		{
			...
			gtk_signal_connect(GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(equalizerwin_save_filesel))), "clicked", GTK_SIGNAL_FUNC(equalizerwin_save_filesel_ok), equalizerwin_save_filesel);
			gtk_signal_connect_object(GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(equalizerwin_save_filesel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(equalizerwin_save_filesel));
		...
		case EQUALIZER_PRESETS_SAVE_TO_WINAMPFILE: {
			...
			gtk_signal_connect(GTK_OBJECT(xmms_gtk_file_selection_get_ok_button(GTK_FILE_SELECTION(save_winamp_filesel))), "clicked", GTK_SIGNAL_FUNC(equalizerwin_save_winamp_filesel_ok), save_winamp_filesel);
			gtk_signal_connect_object(GTK_OBJECT(xmms_gtk_file_selection_get_cancel_button(GTK_FILE_SELECTION(save_winamp_filesel))), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(save_winamp_filesel));
