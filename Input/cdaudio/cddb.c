...
static void cddb_server_dialog_ok_cb(GtkWidget *w, gpointer data)
{
	...
	if (!xmms_gtk_clist_has_selection(GTK_CLIST(server_clist)))
		return;
	pos = xmms_gtk_clist_get_selected_row(GTK_CLIST(server_clist));
...
void cdda_cddb_show_server_dialog(GtkWidget *w, gpointer data)
{
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(server_dialog))), vbox, TRUE, TRUE, 0);
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_action_area(GTK_DIALOG(server_dialog))), bbox, TRUE, TRUE, 0);
...
static gboolean cddb_update_log_window(gpointer data)
{
	...
	if (temp_messages != NULL)
	{
		...
		gtk_clist_moveto(GTK_CLIST(debug_clist),
				 xmms_gtk_clist_row_count(GTK_CLIST(debug_clist)) - 1, -1, 0.5, 0);
		GDK_THREADS_LEAVE();
...
void cdda_cddb_show_network_window(GtkWidget *w, gpointer data)
{
	...
	gtk_clist_moveto(GTK_CLIST(debug_clist), xmms_gtk_clist_row_count(GTK_CLIST(debug_clist)) - 1, -1, 0, 0);
