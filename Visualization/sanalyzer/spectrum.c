...
static void sanalyzer_init(void)
{
	...
	window_gdk = gtk_widget_get_window(window);
	bg_pixmap = gdk_pixmap_create_from_xpm_d(window_gdk, NULL, NULL, sanalyzer_xmms_logo_xpm);
	gdk_window_set_back_pixmap(window_gdk, bg_pixmap, 0);
	gtk_signal_connect(GTK_OBJECT(window),"destroy",GTK_SIGNAL_FUNC(sanalyzer_destroy_cb),NULL);
	gtk_signal_connect(GTK_OBJECT(window), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &window);
	gtk_widget_set_size_request(window, WIDTH, HEIGHT);
	draw_pixmap = gdk_pixmap_new(window_gdk, WIDTH, HEIGHT, -1);
	gc = gdk_gc_new(draw_pixmap);
	
	bar = gdk_pixmap_new(window_gdk, 25, HEIGHT, -1);
	...
	area_gdk = gtk_widget_get_window(area);
	gdk_window_set_back_pixmap(area_gdk, bg_pixmap, 0);
	
	gtk_widget_show(area);
	gtk_widget_show(window);
	gdk_window_clear(window_gdk);
	gdk_window_clear(area_gdk);
}
...
static void sanalyzer_playback_start(void)
{
	if(window)
	{
		GdkWindow *area_gdk = gtk_widget_get_window(area);
		gdk_window_set_back_pixmap(area_gdk, draw_pixmap, 0);
		gdk_window_clear(area_gdk);
	}
}
