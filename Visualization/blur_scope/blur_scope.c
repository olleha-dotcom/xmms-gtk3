...
static void bscope_init(void)
{
	...
	bg_pixmap = gdk_pixmap_create_from_xpm_d(window_gdk, NULL, NULL, bscope_xmms_logo_xpm);
...
static void bscope_render_pcm(gint16 data[2][512])
{
	...
	area_window = gtk_widget_get_window(area);
	if (!area_window)
		return;

	GDK_THREADS_ENTER();
	gdk_draw_indexed_image(area_window, NULL, 0, 0, WIDTH, HEIGHT, GDK_RGB_DITHER_NONE,
